...
let get_parse_error env = 
  ...
  | lazy (Cons (I.Element (state, _, _, _), _)) -> try 
    (Parser_messages.message (I.number state)) 
  with Not_found -> "Invalid syntax (No message for this error)"
...
let rejected =
  SyntaxError (None, "invalid syntax (parser rejected the input)")
