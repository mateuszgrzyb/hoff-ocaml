...
let rec ne_types (t1: type_t) (t2: type_t): bool =
    not (eq_types t1 t2)
  
and eq_types (t1: type_t) (t2: type_t): bool = 
  t1 = t2
  (*
  match (t1, t2) with
...
class ['v] hashtable (name: string) = object
  val name = name
  val _table: (id_t, 'v) Hashtbl.t = Hashtbl.create 10
  
  method add (k: id_t) (v: 'v): unit = 
    Hashtbl.add _table k v

  method remove (k: id_t): unit =
    Hashtbl.remove _table k
  
  method find (k: id_t): 'v =
    try
      Hashtbl.find _table k
    with Not_found -> 
      let scope: string = Hashtbl.fold (fun k _v z -> k ^ "\n" ^ z) _table "" 
      in raise (NameError (name^" "^k^" does not exist within this scope: \n"^scope))

  method find_opt (k: id_t): 'v option = 
    Hashtbl.find_opt _table k
  end
...
let typecheck (ds: g_decl_t list): unit =
  ...
  *)
  ...
  and check_val (id: id_t): type_t =
    ...
    then let (args, rt) = Option.get opt_f in FunT (args, rt) 
